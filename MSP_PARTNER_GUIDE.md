...
> **Nota per chi presenta questa tabella**: ogni riga è stata verificata contro il codice sorgente reale al 19/09/2026. Una funzionalità presente in una versione precedente di questo documento (deception/honeypot nativo) è stata rimossa perché non esiste nella suite — se un prospect tecnico la chiede, la risposta onesta è "sul roadmap, non ancora disponibile", mai "sì, ce l'abbiamo". Il rilevamento "YARA" era in una versione precedente codice presente ma mai collegato al flusso di triage reale, ed era inoltre un set di firme regex interne solo ispirate allo stile YARA, non il motore vero. Entrambi i problemi sono stati risolti: Mjolnir ora usa `yara-python` (i binding ufficiali al motore YARA/libyara reale), compila regole `.yar` reali da `Mjolnir/rules/` (3 regole di partenza incluse: webshell generico, testo da nota di riscatto ransomware, PowerShell codificato) ed è cablato nel flusso di triage. Essendo YARA vero, si possono aggiungere file `.yar` da un qualunque ruleset pubblico (es. Yara-Rules/rules, Neo23x0/signature-base) senza modificare il codice — oggi il set incluso resta comunque piccolo (3 regole), quindi va presentato come "motore YARA reale, copertura di partenza", non come un ruleset commerciale completo.
